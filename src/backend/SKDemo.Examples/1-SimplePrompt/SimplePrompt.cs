namespace SKDemo.Examples
{
    public class SimplePrompt
    {
        public async Task<string> Run(string draft)
        {
            ...
            // var azureFoundryKey = Environment.GetEnvironmentVariable("azureFoundryKey");
            // Debug.Assert(
            //     !string.IsNullOrEmpty(azureFoundryKey),
            //     "azureFoundryKey environment variable is not set."
            // );
            // builder.AddAzureOpenAIChatCompletion(
            //     "gpt-4o-mini", // Azure OpenAI Deployment Name
            //     "https://circuitcircus6541186924.openai.azure.com/", // Azure OpenAI Endpoint
            //     azureFoundryKey
            // ); // Azure OpenAI Key
...
You make awesome job adverts from rough drafts!";
            ...
            var result = await kernel.InvokePromptAsync(prompt, new() { ["input"] = draft });
